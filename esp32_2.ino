...
char* ssid = "ROMERO";
const char* password = "romero2016";
WiFiServer server(80); //Servidor web 

const int LED1 = 5; //Pines de cada LED 
const int LED2 = 4; 
  ...
  if (client) {
    ...
    while (client.connected()) { //Mientras que el cliente está conectado 
      if (client.available()) { //Si se recibe algun dato del cliente
        char c = client.read(); 
        Serial.write(c); //Lee caracter por caracter la petición http 
        linebuf[charcount]=c; 
        ...
        else if (c != '\r') { // Todavía queda un caracter en la línea actual 
